import { TextureCard } from "../Components/Texture_Card";
import type { CardTextureProps } from "../Components/Texture_Card";
  ...
  useEffect(() => {
    const chargementDonnees = async () => {
      ...
      if (!error && data) {
        ...
        setModeles(data as CardTextureProps[]); //fais le lien a la variable apres le debut de la function / on utilise les [] pour dire que c'est une liste
      } else {
        ...
        <div className="h-80 flex px-8 w-full items-end ">
          <div className="flex justify-between items-baseline w-full border-b border-violet-400 pb-4">
            <h1 className="text-4xl font-black text-black uppercase">
              Texture <span className="text-violet-400"> PBR</span>
            </h1>
